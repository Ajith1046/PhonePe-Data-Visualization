...
st.title(" :iphone: PhonePe Pulse Data Visualization")
with st.sidebar:
    select = option_menu(None,
                         ["Home", "Quarter wise Data", "Transaction Data", "Users Data", "Map", 'Contact'],
...
image1 = Image.open(r"C:/Users/Admin/Project_data/download.jpg")
...
lottie_s = load_lottie_url(r"https://assets4.lottiefiles.com/packages/lf20_GXS1DssMnR.json")
lottie_t = load_lottie_url(r"https://assets4.lottiefiles.com/packages/lf20_kdcvbate.json")
lottie_m = load_lottie_url(
    r"https://assets4.lottiefiles.com/datafiles/5FGDT1tGd6PRFWjrlnK36tsX4dv7kt1ihUMebNma/india.json")
# creating dataframes
# path=os.getcwd()+"/pulse/data/aggregated/transaction/country/india/state/"
path = f'C:/Users/Admin/Project_data/pulse/data/aggregated/transaction/country/india/state/'
...
path_tran_user = f'C:/Users/Admin/Project_data/pulse/data/aggregated/user/country/india/state/'
...
path_map_tran = f'C:/Users/Admin/Project_data/pulse/data/map/transaction/hover/country/india/state/'
map_tran_state_list = os.listdir(path_map_tran)
map_tran_list = {'State': [], 'Year': [], 'Quarter': [], 'District': [], 'Transaction_count': [],
                 'Transaction_amount': []}
    ...
    for j in map_tran_yr:
        ...
        for k in map_tran_yr_list:
            ...
            for z in D['data']['hoverDataList']:
                ...
                map_tran_list['Transaction_count'].append(count)
                map_tran_list['Transaction_amount'].append(amount)
...
path_map_user = f'C:/Users/Admin/Project_data/pulse/data/map/user/hover/country/india/state/'
...
path_top_tran = f'C:/Users/Admin/Project_data/pulse/data/top/transaction/country/india/state/'
top_tran_state_list = os.listdir(path_top_tran)
top_tran_list = {'State': [], 'Year': [], 'Quarter': [], 'District': [], 'Transaction_count': [],
                 'Transaction_amount': []}
    ...
    for j in top_tran_yr:
        ...
        for k in top_tran_yr_list:
            ...
            for z in D['data']['districts']:
                ...
                top_tran_list['Transaction_count'].append(count)
                top_tran_list['Transaction_amount'].append(amount)
...
path_top_user = f'C:/Users/Admin/Project_data/pulse/data/top/user/country/india/state/'
...
conn = sqlite3.connect('phonepepulse.db')

create_sql_1 = "CREATE TABLE IF NOT EXISTS agg_tran(State text,Year text,Quarter integer,Transaction_type text,Transaction_count integer,Transaction_amount float)"
cursor = conn.cursor()
cursor.execute(create_sql_1)
for row in Agg_Trans.itertuples():
    insert_sql_1 = f"INSERT INTO agg_tran(State,Year,Quarter,Transaction_type,Transaction_count,Transaction_amount) values ('{row[1]}','{row[2]}',{row[3]},'{row[4]}',{row[5]},{row[6]} )"
...
if select == "Home":
    with st.container():
        ...
        with text_column:
            st.write(
                """The Indian digital payments story has truly captured the world's imagination. From the largest towns to the
                remotest villages, there is a payments revolution being driven by the penetration of mobile phones and data.!!!
    ...
    with st.container():
        ...
        with left_column:
            ...
            if (s1 or st.session_state.s1_state) and x1 == "Transaction Data":
                ...
                x2 = st.selectbox('Select Your Option :white_check_mark:',
                                  ("Year Wise Transaction Data", "State Wise Transaction Data"))
                s2 = st.button("submit")
                ...
                if (s2 or st.session_state.s2_state) and x2 == "Year Wise Transaction Data":
                    ...
                    if s3 or st.session_state.s3_state:
                        ...
                        path = f'C:/Users/Admin/Project_data/pulse/data/aggregated/transaction/country/india/{x3}/{x4}.json'
                    ...
                    st.session_state.s2_state = True
                    s_n = st.selectbox("Select the State :round_pushpin: :white_check_mark:", (
                    ...
                    if s4 or st.session_state.s4_state:
                        ...
                        path1 = f'C:/Users/Admin/Project_data/pulse/data/aggregated/transaction/country/india/state/{s_n}/{y}/{Q}.json'
            ...
            if "s1_state" not in st.session_state:
                ...
                st.session_state.s1_state = True
                x2 = st.selectbox('Select Your Option :white_check_mark:',
                                  ("Year Wise User Data", "State Wise User Data"))
                s2 = st.button("submit")
                if "s2_state" not in st.session_state:
                    st.session_state.s2_state = False
                ...
                if (s2 or st.session_state.s2_state) and x2 == 'Year Wise User Data':
                    ...
                    if s3 or st.session_state.s3_state:
                        st.session_state.s3_state = True
                        path2 = f'C:/Users/Admin/Project_data/pulse/data/aggregated/user/country/india/{x3}/{x4}.json'
                ...
                if "s2_state" not in st.session_state:
                    ...
                    if s4 or st.session_state.s4_state:
                        st.session_state.s4_state = True
                        path3 = f'C:/Users/Admin/Project_data/pulse/data/aggregated/user/country/india/state/{s_n}/{y}/{Q}.json'
...
if select == "Contact":
    aboutme = """ I am interested in pursuing a career in Machine Learning
                and working towards becoming a professional in
                this exciting and rapidly evolving field.!"""
    links={
        "GITHUB": "https://github.com/Ajith1046",
        "LINKEDIN": "https://www.linkedin.com/in/ajith-h-9332891a1/"}
    column1, column2 = st.columns(2)
    with column1:
        column1.image(Image.open(r"C:/Users/Admin/Downloads/me.jpeg"),width=150)
    with column2:
        st.subheader("AJITH H")
        st.subheader(f'{"Mail :"}  {"ajithh10@gmail.com"}')
        st.write(aboutme)
...
if select == "Transaction Data":
    ...
    if op_1 == "Aggregate Transaction":
        ...
        with col1:
            ...
            if transaction_type:
                cursor.execute(
                    f"SELECT DISTINCT State,Quarter,Year,Transaction_type,Transaction_count,Transaction_amount FROM agg_tran WHERE Transaction_type = '{transaction_type}' ORDER BY State,Quarter,Year")
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=['State', 'Quarter', 'Year', 'Transaction_type', 'Transaction_count',
                                           'Transaction_amount'])
                st.write(df)
        with col2:
            ...
            if transaction_type and select_year:
                cursor.execute(
                    f"SELECT DISTINCT State,Year,Quarter,Transaction_type,Transaction_count,Transaction_amount FROM agg_tran WHERE Year = '{select_year}' AND Transaction_type = '{transaction_type}' ORDER BY State,Quarter,Year")
                df = pd.DataFrame(cursor.fetchall(),
        ...
        with col3:
            ...
            if transaction_type and select_state and select_year:
                cursor.execute(
                    f"SELECT DISTINCT State,Year,Quarter,Transaction_type,Transaction_count,Transaction_amount FROM agg_tran WHERE State = '{select_state}' AND Transaction_type = '{transaction_type}' And Year = '{select_year}' ORDER BY State,Quarter,Year")
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=['State', 'Year', "Quarter", 'Transaction_type', 'Transaction_count',
                                           'Transaction_amount'])
                st.write(df)
    ...
    if op_1 == "District wise transaction":
        ...
        with col1:
            ...
            if select_state:
                cursor.execute(
                    f"SELECT DISTINCT State,Year,Quarter,District,Transaction_count,Transaction_amount FROM map_tran WHERE State = '{select_state}' ORDER BY State,Year,Quarter,District");
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=['State', 'Year', "Quarter", 'District', 'Transaction_count',
                                           'Transaction_amount'])
                st.write(df)
        with col2:
            ...
            if select_year and select_state:
                cursor.execute(
                    f"SELECT DISTINCT State,Year,Quarter,District,Transaction_count,Transaction_amount FROM map_tran WHERE Year = '{select_year}' AND State = '{select_state}' ORDER BY State,Year,Quarter,District");
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=['State', 'Year', "Quarter", 'District', 'Transaction_count',
                                           'Transaction_amount'])
                st.write(df)
        with col3:
            ...
            if district and select_state and select_year:
                cursor.execute(
                    f"SELECT DISTINCT State,Year,Quarter,District,Transaction_count,Transaction_amount FROM map_tran WHERE District = '{district}' AND State = '{select_state}' AND Year = '{select_year}' ORDER BY State,Year,Quarter,District");
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=['State', 'Year', "Quarter", 'District', 'Transaction_count',
                                           'Transaction_amount'])
                st.write(df)
    ...
    if op_1 == "Top Transaction":
        ...
        with col1:
            ...
            if select_state:
                cursor.execute(
                    f"SELECT State, Year, Quarter, District, Transaction_count, Transaction_amount FROM top_tran WHERE State = '{select_state}' GROUP BY State, Year, Quarter")
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=["State", "Year", "Quarter", "District", "Transaction_count",
                                           "Transaction_amount"])
                st.write(df)
        with col2:
            ...
            if select_state and select_year:
                cursor.execute(
                    f"SELECT State, Year, Quarter, District, Transaction_count, Transaction_amount FROM top_tran WHERE State = '{select_state}' AND Year = '{select_year}' GROUP BY State, Year, Quarter")
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=["State", "Year", "Quarter", "District", "Transaction_count",
                                           "Transaction_amount"])
                st.write(df)
        ...
        with col3:
            ...
            if select_state and select_year and select_quarter:
                cursor.execute(
                    f"SELECT State, Year, Quarter, District, Transaction_count, Transaction_amount FROM top_tran WHERE State = '{select_state}' AND Year = '{select_year}' AND Quarter = '{select_quarter}' GROUP BY State, Year, Quarter")
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=["State", "Year", "Quarter", "District", "Transaction_count",
                                           "Transaction_amount"])
                st.write(df)
...
if select == "Users Data":
    ...
    if op_2 == "District wise users":
        ...
        with col1:
            ...
            if select_state:
                cursor.execute(
                    f"SELECT State,Year,Quarter,District,Registered_Users FROM map_users WHERE State = '{select_state}' ORDER BY State,Year,Quarter,District")
                df = pd.DataFrame(cursor.fetchall(),
        ...
        with col2:
            ...
            if select_state and select_year:
                cursor.execute(
                    f"SELECT State,Year,Quarter,District,Registered_Users FROM map_users WHERE Year = '{select_year}' AND State = '{select_state}' ORDER BY State,Year,Quarter,District")
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=['State', 'Year', "Quarter", 'District', 'Registered_Users'])
                st.write(df)
        with col3:
            ...
            if select_state and select_year and district:
                cursor.execute(
                    f"SELECT State,Year,Quarter,District,Registered_Users FROM map_users WHERE Year = '{select_year}' AND State = '{select_state}' AND District = '{district}' ORDER BY State,Year,Quarter,District")
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=['State', 'Year', "Quarter", 'District', 'Registered_Users'])
                st.write(df)
    ...
    if op_2 == "Aggregate brand users":
        ...
        if brand_type:
            cursor.execute(
                f"SELECT State,Year,Quarter,user_device_brand,user_count,user_percentage FROM agg_users WHERE user_device_brand='{brand_type}' ORDER BY State,Year,Quarter,user_device_brand,user_count,user_percentage DESC")
            df = pd.DataFrame(cursor.fetchall(), columns=['State', 'Year', 'Quarter', 'user_device_brand', 'user_count',
                                                          'user_percentage'])
            with col1:
                st.write(df)
        if brand_type and select_year:
            cursor.execute(
                f"SELECT State,Year,Quarter,user_device_brand,user_count,user_percentage FROM agg_users WHERE Year='{select_year}' AND user_device_brand='{brand_type}' ORDER BY State,Year,Quarter,user_device_brand,user_count,user_percentage DESC")
            df = pd.DataFrame(cursor.fetchall(), columns=['State', 'Year', 'Quarter', 'user_device_brand', 'user_count',
                                                          'user_percentage'])
            with col2:
                st.write(df)
        if brand_type and select_year and select_state:
            cursor.execute(
                f"SELECT State,Year,Quarter,user_device_brand,user_count,user_percentage FROM agg_users WHERE State='{select_state}' AND Year='{select_year}' AND user_device_brand='{brand_type}' ORDER BY State,Year,Quarter,user_device_brand,user_count,user_percentage DESC")
            df = pd.DataFrame(cursor.fetchall(), columns=['State', 'Year', 'Quarter', 'user_device_brand', 'user_count',
                                                          'user_percentage'])
            with col3:
                st.write(df)
    ...
    if op_2 == "Top users":
        ...
        with col1:
            ...
            if select_state:
                cursor.execute(
                    f"SELECT State, Year, Quarter, District, Registered_Users FROM top_users WHERE State ='{select_state}' GROUP BY State, Year, Quarter")
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=["State", "Year", "Quarter", "District", "Registered_Users"])
                st.write(df)
        with col2:
            ...
            if select_state and select_year:
                cursor.execute(
                    f"SELECT State, Year, Quarter, District, Registered_Users FROM top_users WHERE State = '{select_state}' AND Year = '{select_year}' GROUP BY State, Year, Quarter")
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=["State", "Year", "Quarter", "District", "Registered_Users"])
                st.write(df)
        with col3:
            ...
            if select_state and select_year and select_quarter:
                cursor.execute(
                    f"SELECT State, Year, Quarter, District, Registered_Users FROM top_users WHERE State = '{select_state}' AND Year = '{select_year}' AND Quarter = '{select_quarter}' GROUP BY State, Year, Quarter")
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=["State", "Year", "Quarter", "District", "Registered_Users"])
                st.write(df)
...
if select == 'Map':
    ...
    shapefile_path = r"C:/Users/Admin/Project_data/state boundary/India-State-and-Country-Shapefile-Updated-Jan-2020/India_State_Boundary.shp"
